...
class NotificationController extends Controller
{
    ...
    public function getPageAll(Notification $notification){
        ...
        $this->data['notifies'] = $notification->where(['user_id'=> $user_id])->paginate(10);
        return view('account.notify.all', $this->data);
    }

    public function getPageNew(Notification $notification){
        $user_id = Auth::user()->id;
        $this->data['notifies'] = $notification->where(['user_id'=> $user_id, 'status' => 1])->paginate(10);
